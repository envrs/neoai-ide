...
async function getOpenAIAdapter() {
  const { OpenAIAdapter } = await import("@neopilot/runtime");
...
async function getAnthropicAdapter() {
  const { AnthropicAdapter } = await import("@neopilot/runtime");
  return new AnthropicAdapter({ model: "claude-3-7-sonnet-20250219" });
}

async function getGeminiAdapter() {
  const { GoogleGenerativeAIAdapter } = await import("@neopilot/runtime");
  return new GoogleGenerativeAIAdapter();
}

async function getGroqAdapter() {
  const { GroqAdapter } = await import("@neopilot/runtime");
  return new GroqAdapter();
}

async function getLangChainOpenAIAdapter() {
  const { LangChainAdapter } = await import("@neopilot/runtime");
...
async function getBedrockAdapter() {
  const { BedrockAdapter } = await import("@neopilot/runtime");
