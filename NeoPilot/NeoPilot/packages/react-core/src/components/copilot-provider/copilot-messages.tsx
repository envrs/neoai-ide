/**
 * An internal context to separate the messages state (which is constantly changing) from the rest of NeoPilot context
 */
...
import {
  ...
  Message,
  GraphQLError,
} from "@neopilot/runtime-client-gql";
...
import {
  ...
  NeoPilotRemoteEndpointDiscoveryError,
...
function shouldShowAsBanner(gqlError: GraphQLError): boolean {
  ...
  const code = extensions.code as NeoPilotErrorCode;
  if (
    code === NeoPilotErrorCode.AGENT_NOT_FOUND ||
    code === NeoPilotErrorCode.API_NOT_FOUND ||
    code === NeoPilotErrorCode.REMOTE_ENDPOINT_NOT_FOUND ||
    code === NeoPilotErrorCode.CONFIGURATION_ERROR ||
    code === NeoPilotErrorCode.MISSING_PUBLIC_API_KEY_ERROR ||
    code === NeoPilotErrorCode.UPGRADE_REQUIRED_ERROR
  ) {
    return true;
  }
  ...
  if (originalError?.stack) {
    return (
      ...
      originalError.stack.includes("NeoPilotRemoteEndpointDiscoveryError") ||
      originalError.stack.includes("NeoPilotAgentDiscoveryError")
    );
  }
...
export function CopilotMessages({ children }: { children: ReactNode }) {
  ...
  const traceUIError = useCallback(
    async (error: NeoPilotError, originalError?: any) => {
  ...
  const createStructuredError = (gqlError: GraphQLError): NeoPilotError | null => {
    const extensions = gqlError.extensions;
    const originalError = extensions?.originalError as any;
    ...
    if (originalError?.stack) {
      if (originalError.stack.includes("CopilotApiDiscoveryError")) {
        return new NeoPilotApiDiscoveryError({ message: originalError.message });
      }
      if (originalError.stack.includes("NeoPilotRemoteEndpointDiscoveryError")) {
        return new NeoPilotRemoteEndpointDiscoveryError({ message: originalError.message });
      }
      if (originalError.stack.includes("NeoPilotAgentDiscoveryError")) {
        return new NeoPilotAgentDiscoveryError({
          agentName: "",
          availableAgents: [],
        });
      }
    ...
    if (code) {
      return new NeoPilotError({ message, code });
    }
    ...
    (error: any) => {
      if (error.graphQLErrors?.length) {
        ...
        const routeError = (gqlError: GraphQLError) => {
          ...
          if (!isDev) {
            console.error("NeoPilot Error (hidden in production):", gqlError.message);
            return;
          }

          // Silent errors - just log
          if (visibility === ErrorVisibility.SILENT) {
            console.error("NeoPilot Silent Error:", gqlError.message);
            return;
          }
          ...
          if (ckError) {
            ...
            const fallbackError = new NeoPilotError({
        ...
        const isDev = shouldShowDevConsole(showDevConsole);
        if (!isDev) {
          console.error("NeoPilot Error (hidden in production):", error);
        } else {
