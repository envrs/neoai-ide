...
export type UseChatOptions = {
  ...
  /**
   * The NeoPilot API configuration.
   */
      ...
      try {
        while (true) {
          ...
          (metaEvents ?? []).forEach((ev) => {
            ...
            if (ev.name === MetaEventName.NeoPilotLangGraphInterruptEvent) {
              const data = (ev as NeoPilotLangGraphInterruptEvent).data;
          ...
          if (
            ...
            const guardrailsError = new NeoPilotError({
          ...
          if (
            ...
            let errorCode = NeoPilotErrorCode.NETWORK_ERROR;
            if (originalCode && Object.values(NeoPilotErrorCode).includes(originalCode)) {
              errorCode = originalCode;
            }

            // Create a structured NeoPilotError preserving original error information
            const structuredError = new NeoPilotError({
