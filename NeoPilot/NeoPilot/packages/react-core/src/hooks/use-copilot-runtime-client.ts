...
export const useCopilotRuntimeClient = (options: CopilotRuntimeClientHookOptions) => {
  ...
  const traceUIError = async (error: NeoPilotError, originalError?: any) => {
    try {
  ...
  const runtimeClient = useMemo(() => {
    return new CopilotRuntimeClient({
      ...
      handleGQLWarning: (message: string) => {
        ...
        const warningError = new NeoPilotError({
          message,
          code: NeoPilotErrorCode.UNKNOWN,
        });
...
function createStructuredError(gqlError: GraphQLError): NeoPilotError | null {
  const extensions = gqlError.extensions;
  ...
  if (originalError?.stack?.includes("CopilotApiDiscoveryError")) {
    return new NeoPilotApiDiscoveryError({ message });
  }
  if (originalError?.stack?.includes("NeoPilotRemoteEndpointDiscoveryError")) {
    return new NeoPilotRemoteEndpointDiscoveryError({ message });
  }
