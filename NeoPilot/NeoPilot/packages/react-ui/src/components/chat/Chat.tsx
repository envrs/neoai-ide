/**
 * <br/>
 * <img src="https://cdn.neopilot.ai/docs/neopilot/images/CopilotChat.gif" width="500" />
 *
 * A chatbot panel component for the NeoPilot framework. The component allows for a high degree
...
import type { SuggestionItem } from "@neopilot/react-core";
import {
  NeoPilotError,
  NeoPilotErrorCode,
  CopilotErrorEvent,
...
import { HintFunction, runAgent, stopAgent } from "@neopilot/react-core";
...
export interface CopilotChatProps {
  ...
  /**
   * Disables inclusion of NeoPilot’s default system message. When true, no system message is sent (this also suppresses any custom message from <code>makeSystemMessage</code>).
   */
  ...
  /**
   * Event hooks for NeoPilot chat events.
...
export function CopilotChat({
  ...
  useEffect(() => {
    ...
    const handlePaste = async (e: ClipboardEvent) => {
      ...
      if (!target.parentElement?.classList.contains("neoPilotInput")) return;
  ...
  if (!chatContext) {
    return (
      <ChatContextProvider icons={icons} labels={labels} open={true} setOpen={() => {}}>
        <div className={`neoPilotChat ${className ?? ""}`}>{children}</div>
      </ChatContextProvider>
    );
  }
  return <>{children}</>;
