...
import { useCopilotContext, useCopilotMessagesContext } from "@neopilot/react-core";
import {
  getPublishedNeoPilotVersion,
  logActions,
  logMessages,
  logReadables,
...
export function CopilotDevConsole() {
  ...
  const checkForUpdates = (force: boolean = false) => {
    ...
    getPublishedNeoPilotVersion(currentVersion, force)
      .then((v) => {
  ...
  return (
    <div
      ...
      className={
        "neoPilotDevConsole " +
        (versionStatus === "update-available" ? "neoPilotDevConsoleUpgrade" : "") +
        (versionStatus === "outdated" ? "neoPilotDevConsoleWarnOutdated" : "")
      }
    >
      <VersionInfo
...
function VersionInfo({
  ...
  const installCommand = [
    `npm install`,
    `@neopilot/react-core@${latestVersion}`,
    `@neopilot/react-ui@${latestVersion}`,
    `@neopilot/react-textarea@${latestVersion}`,
    `&& npm install @neopilot/runtime@${latestVersion}`,
  ].join(" ");
  ...
  if (versionStatus === "update-available" || versionStatus === "outdated") {
    return (
      <div className="neoPilotVersionInfo">
        <p>
...
export default function DebugMenuButton({
  ...
  return (
    <>
      <Menu>
        <MenuButton
          className={`neoPilotDebugMenuTriggerButton ${mode === "compact" ? "compact" : ""}`}
        >
        ...
        <MenuItems
          transition
          anchor="bottom end"
          className="neoPilotDebugMenu"
          style={{ zIndex: 40 }}
        >
          <MenuItem>
            <button className="neoPilotDebugMenuItem" onClick={() => logReadables(context)}>
              Log Readables
            </button>
          </MenuItem>
          <MenuItem>
            <button className="neoPilotDebugMenuItem" onClick={() => logActions(context)}>
              Log Actions
            </button>
          </MenuItem>
          <MenuItem>
            <button
              className="neoPilotDebugMenuItem"
          ...
          <MenuItem>
            <button className="neoPilotDebugMenuItem" onClick={() => checkForUpdates(true)}>
              Check for Updates
            </button>
          </MenuItem>
          <hr />
          <MenuItem>
            <button className="neoPilotDebugMenuItem" onClick={() => setShowDevConsole(false)}>
              Hide Dev Console
            </button>
          </MenuItem>
        </MenuItems>
