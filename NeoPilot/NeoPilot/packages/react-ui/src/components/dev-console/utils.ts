...
import { NeoPilotVersion } from "./types";
export { shouldShowDevConsole } from "@neopilot/react-core";

export async function getPublishedNeoPilotVersion(
  current: string,
  forceCheck: boolean = false,
): Promise<NeoPilotVersion> {
  const LOCAL_STORAGE_KEY = "__neopilot_version_check__";
  const serializedVersion = localStorage.getItem(LOCAL_STORAGE_KEY);
  if (serializedVersion && !forceCheck) {
    try {
      const parsedVersion: NeoPilotVersion = JSON.parse(serializedVersion);
      ...
      console.error("Failed to parse NeoPilotVersion from localStorage", error);
    }
  }

  try {
    const response = await fetch("https://api.cloud.neopilot.ai/check-for-updates", {
      method: "POST",
      headers: {
