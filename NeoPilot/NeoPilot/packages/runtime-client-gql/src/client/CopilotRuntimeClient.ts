    ...
    try {
      ...
      const mismatch = publicApiKey
        ...
        : await getPossibleVersionMismatch({
            runtimeVersion: result.headers.get("X-NeoPilot-Runtime-Version")!,
            runtimeClientGqlVersion: packageJson.version,
          });
      if (result.status !== 200) {
        if (result.status >= 400 && result.status <= 500) {
          if (mismatch) {
            throw new NeoPilotVersionMismatchError(mismatch);
          }

          throw new ResolvedNeoPilotError({ status: result.status });
        }
      }
      ...
      throw new NeoPilotLowLevelError({ error: error as Error, url: args[0] as string });
    }
  };
  ...
  constructor(options: CopilotRuntimeClientOptions) {
    ...
    this.client = new Client({
      ...
      fetchOptions: {
        headers: {
          ...headers,
          "X-NeoPilot-Runtime-Client-GQL-Version": packageJson.version,
        },
      ...
      start(controller) {
        source.subscribe(({ data, hasNext, error }) => {
          if (error) {
            ...
            // Handle structured errors specially - check if it's a NeoPilotError with visibility
