import { NeoPilotLowLevelError, isStructuredNeoPilotError } from "@neopilot/shared";
    ...
    return this.eventStream$.pipe(
      ...
      mergeMap((acc): RuntimeEvent[] => {
        ...
        if (acc.event.event == LangGraphEventTypes.OnChatModelStream) {
          if ("neopilot:emit-tool-calls" in (acc.event.metadata || {})) {
            shouldEmitToolCalls = acc.event.metadata["neopilot:emit-tool-calls"];
          }
          if ("neopilot:emit-messages" in (acc.event.metadata || {})) {
            shouldEmitMessages = acc.event.metadata["neopilot:emit-messages"];
          }
        }
        ...
        if (acc.event.event === LangGraphEventTypes.OnNeoPilotInterrupt) {
          events.push({
            type: RuntimeEventTypes.MetaEvent,
            name: RuntimeMetaEventName.NeoPilotLangGraphInterruptEvent,
            data: acc.event.data,
          });
        }

        // Handle NeoPilot error events with preserved semantic information
        if (acc.event.event === LangGraphEventTypes.OnNeoPilotError) {
        ...
        switch (acc.event!.event) {
          ...
          case LangGraphEventTypes.OnCustomEvent:
            ...
            //
            if (acc.event.name === CustomEventNames.NeoPilotManuallyEmitMessage) {
              events.push({
            ...
            //
            else if (acc.event.name === CustomEventNames.NeoPilotManuallyEmitToolCall) {
              events.push({
            ...
            break;
          case LangGraphEventTypes.OnNeoPilotStateSync:
            events.push({
      ...
      catchError((error) => {
        // If it's a structured NeoPilotError, re-throw it to be handled by the frontend error system
        if (isStructuredNeoPilotError(error)) {
          throw error;
        }
