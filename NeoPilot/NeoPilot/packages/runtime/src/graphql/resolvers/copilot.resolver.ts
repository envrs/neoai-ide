...
import {
  NeoPilotLangGraphInterruptEvent,
...
import { randomId } from "@neopilot/shared";
import { AgentsResponse } from "../types/agents-response.type";
import { LangGraphEventTypes } from "../../agents/langgraph/events";
import {
  NeoPilotError,
  NeoPilotLowLevelError,
  isStructuredNeoPilotError,
...
export class CopilotResolver {
  ...
  async availableAgents(@Ctx() ctx: GraphQLContext) {
    ...
    const agentsWithEndpoints = await ctx._neopilot.runtime.discoverAgentsFromEndpoints(ctx);
  ...
  async generateCopilotResponse(
    ...
    telemetry.capture("oss.runtime.copilot_request_created", {
      ...
      ...(ctx.request.headers.get("x-copilotcloud-public-api-key")
        ...
        : {}),
      ...(ctx._neopilot.baseUrl
        ? {
            "cloud.base_url": ctx._neopilot.baseUrl,
          }
        : {
            "cloud.base_url": "https://api.cloud.neopilot.ai",
          }),
    });
    ...
    const copilotRuntime = ctx._neopilot.runtime;
    const serviceAdapter = ctx._neopilot.serviceAdapter;
    ...
    if (data.cloud) {
      ...
      } else if (ctx._neopilot.cloud?.baseUrl) {
        copilotCloudBaseUrl = ctx._neopilot.cloud?.baseUrl;
      } else {
        copilotCloudBaseUrl = "https://api.cloud.neopilot.ai";
      }
    ...
    try {
      ...
      // Catch structured NeoPilot errors at the main mutation level and re-throw as GraphQL errors
      if (isStructuredNeoPilotError(error) || (error as any)?.extensions?.visibility) {
      ...
      throw error; // Re-throw non-NeoPilot errors as-is
    }
    ...
    const response = {
      ...
      metaEvents: new Repeater(async (push, stop) => {
        ...
        eventStreamSubscription = eventStream.subscribe({
          next: async (event) => {
            ...
            switch (event.name) {
              ...
              case RuntimeMetaEventName.LangGraphInterruptEvent:
                ...
                break;
              case RuntimeMetaEventName.NeoPilotLangGraphInterruptEvent:
                push(
                  plainToInstance(NeoPilotLangGraphInterruptEvent, {
                    type: event.type,
                    name: event.name,
                    data: {
          ...
          error: (err) => {
            // For structured NeoPilot errors, set proper error response status
            if (err?.name?.includes("NeoPilot") || err?.extensions?.visibility) {
              responseStatus$.next(
      ...
      messages: new Repeater(async (pushMessage, stopStreamingMessages) => {
        ...
        eventStreamSubscription = eventStream.subscribe({
          ...
          error: (err) => {
            // For structured NeoPilot errors, set proper error response status
            if (
              err instanceof NeoPilotError ||
              err instanceof NeoPilotLowLevelError ||
              (err instanceof Error && err.name && err.name.includes("NeoPilot")) ||
