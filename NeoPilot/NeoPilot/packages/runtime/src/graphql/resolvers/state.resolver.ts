...
import { NeoPilotAgentDiscoveryError } from "@neopilot/shared";
...
export class StateResolver {
  ...
  async loadAgentState(@Ctx() ctx: GraphQLContext, @Arg("data") data: LoadAgentStateInput) {
    const agents = await ctx._neopilot.runtime.getAllAgents(ctx);
    const hasAgent = agents.some((agent) => agent.name === data.agentName);
    if (!hasAgent) {
      throw new NeoPilotAgentDiscoveryError({
        agentName: data.agentName,
        availableAgents: agents.map((a) => ({ name: a.name, id: a.name })),
      });
    }

    const state = await ctx._neopilot.runtime.loadAgentState(ctx, data.threadId, data.agentName);
