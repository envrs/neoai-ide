 ...
 *
 * ```tsx
 * import { CopilotRuntime } from "@neopilot/runtime";
 *
...
import {
  ...
  Parameter,
  ResolvedNeoPilotError,
  NeoPilotApiDiscoveryError,
  randomId,
  NeoPilotError,
  NeoPilotAgentDiscoveryError,
  NeoPilotMisuseError,
  NeoPilotErrorCode,
  NeoPilotLowLevelError,
  CopilotErrorHandler,
  CopilotErrorEvent,
  CopilotRequestContext,
  ensureStructuredError,
} from "@neopilot/shared";
import {
...
import { langchainMessagesToNeoPilot } from "./remote-lg-action";
...
export interface CopilotRuntimeConstructorParams<T extends Parameter[] | [] = []> {
   ...
   */
  remoteActions?: NeoPilotEndpoint[];
   ...
   * Requires publicApiKey from NeoPilot component to be set:
   *
   * ```tsx
   * <NeoPilot publicApiKey="ck_pub_..." />
   * ```
   *
   ...
   * (e.g., `@neopilot/runtime`, `ai`) to establish a connection.
  ...
  constructor(params?: CopilotRuntimeConstructorParams<T>) {
    if (
      ...
      throw new NeoPilotMisuseError({
        message:
          "LangGraph Platform remote endpoints are deprecated in favor of the `agents` property. Refer to https://docs.neopilot.ai/langgraph for more information.",
      });
    }
    ...
    //   })
    // ) {
    //   console.warn('LangGraph Agent class should be imported from @neopilot/runtime. ')
    // }
...
please use an LLM adapter instead.`,
      ...
      const actionInputs = flattenToolCallsNoDuplicates([
        ...
        ...clientSideActionsInput.filter(
          // Filter remote actions from NeoPilot core loop
      ...
      let structuredError: NeoPilotError;

      if (error instanceof NeoPilotError) {
        structuredError = error;
      } else {
        // Convert non-NeoPilotErrors to structured errors, but preserve already structured ones
  ...
  async discoverAgentsFromEndpoints(graphqlContext: GraphQLContext): Promise<AgentWithEndpoint[]> {
    const agents: Promise<AgentWithEndpoint[]> = this.remoteEndpointDefinitions.reduce(
      async (acc: Promise<Agent[]>, endpoint) => {
        ...
        if (endpoint.type === EndpointType.LangGraphPlatform) {
          ...
          try {
            ...
            if (data && "detail" in data && (data.detail as string).toLowerCase() === "not found") {
              throw new NeoPilotAgentDiscoveryError({ availableAgents: this.availableAgents });
            }
          } catch (e) {
            throw new NeoPilotMisuseError({
              message: `
              ...
              See more: https://docs.neopilot.ai/troubleshooting/common-issues`,
            });
          }
        ...
        const cpkEndpoint = endpoint as NeoPilotEndpoint;
        ...
        try {
          ...
          if (!response.ok) {
            if (response.status === 404) {
              throw new NeoPilotApiDiscoveryError({ url: fetchUrl });
            }
            throw new ResolvedNeoPilotError({
          ...
          throw new NeoPilotLowLevelError({ error: error as Error, url: fetchUrl });
        }
      },
      Promise.resolve([]),
    );
  ...
  async loadAgentState(
    ...
    if (
      "endpoint" in agent &&
      (agent.endpoint.type === EndpointType.NeoPilot || !("type" in agent.endpoint))
    ) {
      const cpkEndpoint = agent.endpoint as NeoPilotEndpoint;
      ...
      try {
        ...
        throw new NeoPilotLowLevelError({ error, url: fetchUrl });
      }
    }
    ...
    try {
      ...
      if ("endpoint" in agent && agent.endpoint.type === EndpointType.LangGraphPlatform) {
        ...
        const aguiAgent = graphqlContext._neopilot.runtime.agents[agent.name] as LangGraphAgent;
        if (!aguiAgent) {
      ...
      if (errorStatus === 404) {
        ...
        throw new ResolvedNeoPilotError({
          ...
          code: NeoPilotErrorCode.CONFIGURATION_ERROR,
        });
      }
    }
    ...
    if (Object.keys(state).length === 0) {
      ...
      const neopilotMessages = langchainMessagesToNeoPilot(messages);
      return {
        threadId: threadId || "",
        threadExists: true,
        state: JSON.stringify(stateWithoutMessages),
        messages: JSON.stringify(neopilotMessages),
      };
    }
  ...
  private async processAgentRequest(
    ...
    if (!currentAgent) {
      throw new NeoPilotAgentDiscoveryError({ agentName, availableAgents: this.availableAgents });
    }
      ...
      eventSource.stream(async (eventStream$) => {
        from(stream).subscribe({
          ...
          error: async (err) => {
            ...
            // Preserve structured NeoPilot errors, only convert unstructured errors
  ...
  private convertStreamingErrorToStructured(error: any): NeoPilotError {
    // Determine a more helpful error message based on context
    let helpfulMessage = generateHelpfulErrorMessage(error, "agent streaming connection");

    // For network-related errors, use NeoPilotLowLevelError to preserve the original error
    if (
      ...
      return new NeoPilotLowLevelError({
    ...
    // For all other errors, preserve the raw error in a basic NeoPilotError
    return new NeoPilotError({
      message: helpfulMessage,
      code: NeoPilotErrorCode.UNKNOWN,
    });
  }
  ...
  private async error(
    ...
    if (!publicApiKey) {
      if (!this.hasWarnedAboutError) {
        console.warn(
          "NeoPilot: onError handler provided but requires publicApiKey to be defined for error handling to work.",
        );
...
export function neoPilotEndpoint(config: Omit<NeoPilotEndpoint, "type">): NeoPilotEndpoint {
  return {
    ...config,
    type: EndpointType.NeoPilot,
  };
}
