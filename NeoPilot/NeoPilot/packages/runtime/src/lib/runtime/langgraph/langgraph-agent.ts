...
interface NeoPilotStateEnrichment {
  neopilot: {
...
export enum CustomEventNames {
  NeoPilotManuallyEmitMessage = "neopilot_manually_emit_message",
  NeoPilotManuallyEmitToolCall = "neopilot_manually_emit_tool_call",
  NeoPilotManuallyEmitIntermediateState = "neopilot_manually_emit_intermediate_state",
  NeoPilotExit = "neopilot_exit",
}
...
export class LangGraphAgent extends AGUILangGraphAgent {
  ...
  dispatchEvent(event: ProcessedEvents) {
    if (event.type === EventType.CUSTOM) {
      ...
      if (customEvent.name === CustomEventNames.NeoPilotManuallyEmitMessage) {
      ...
      if (customEvent.name === CustomEventNames.NeoPilotManuallyEmitToolCall) {
        this.subscriber.next({
      ...
      if (customEvent.name === CustomEventNames.NeoPilotManuallyEmitIntermediateState) {
      ...
      if (customEvent.name === CustomEventNames.NeoPilotExit) {
        this.subscriber.next({
    ...
    if ("neopilot:emit-tool-calls" in (rawEvent.metadata || {})) {
      if (rawEvent.metadata["neopilot:emit-tool-calls"] === false && isToolEvent) {
        return false;
      }
    }
    if ("neopilot:emit-messages" in (rawEvent.metadata || {})) {
      if (rawEvent.metadata["neopilot:emit-messages"] === false && isMessageEvent) {
        return false;
      }
    }
      ...
      map((processedEvent) => {
        ...
        if (processedEvent.type === EventType.RAW) {
          ...
          const toolCallUsedToPredictState = event.metadata?.[
            "neopilot:emit-intermediate-state"
          ]?.some(
          ...
          if (eventType === LangGraphEventTypes.OnChatModelStream && toolCallUsedToPredictState) {
            return {
              type: EventType.CUSTOM,
              name: "PredictState",
              value: event.metadata["neopilot:emit-intermediate-state"],
            };
          }
        }
  ...
  langGraphDefaultMergeState(
    ...
    input: RunAgentInput,
  ): State<StateEnrichment & NeoPilotStateEnrichment> {
