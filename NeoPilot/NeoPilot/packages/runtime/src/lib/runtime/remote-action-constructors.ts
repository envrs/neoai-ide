...
import { Action } from "@neopilot/shared";
import { execute } from "./remote-lg-action";
import { NeoPilotError, NeoPilotLowLevelError } from "@neopilot/shared";
import { writeJsonLineResponseToEventStream } from "../streaming";
import { NeoPilotApiDiscoveryError, ResolvedNeoPilotError } from "@neopilot/shared";
import { parseJson, tryMap } from "@neopilot/shared";
  ...
  const agents = endpoint.agents.map((agent) => ({
    ...
    remoteAgentHandler: async ({
      ...
      try {
        ...
        // Preserve structured NeoPilot errors with semantic information
        if (error instanceof NeoPilotError || error instanceof NeoPilotLowLevelError) {
...
export function constructRemoteActions({
  ...
  url: string;
  onBeforeRequest?: NeoPilotEndpoint["onBeforeRequest"];
  graphqlContext: GraphQLContext;
  logger: Logger;
  messages: Message[];
  agentStates?: AgentStateInput[];
