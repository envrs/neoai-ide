import { Action, NeoPilotErrorCode } from "@neopilot/shared";
...
export type EndpointDefinition = NeoPilotEndpoint | LangGraphPlatformEndpoint;
...
export interface NeoPilotEndpoint extends BaseEndpointDefinition<EndpointType.NeoPilot> {
  url: string;
...
export async function setupRemoteActions({
  ...
  const filtered = remoteEndpointDefinitions.filter((value, index, self) => {
    ...
    return index === self.findIndex((t: NeoPilotEndpoint) => t.url === value.url);
  });
