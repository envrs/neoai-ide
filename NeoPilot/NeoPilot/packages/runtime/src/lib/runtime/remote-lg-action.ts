...
import { isValidUUID, randomUUID } from "@neopilot/shared";
...
export function isUserConfigurationError(error: any): boolean {
  return (
    (error instanceof NeoPilotError || error instanceof NeoPilotLowLevelError) &&
  ...
  return new ReadableStream({
    async start(controller) {
      ...
      if (errorCode === "ECONNREFUSED") {
        ...
        // Preserve already structured NeoPilot errors with semantic information
        if (
          lastError instanceof NeoPilotError ||
          lastError instanceof NeoPilotLowLevelError ||
          (lastError instanceof Error && lastError.name && lastError.name.includes("NeoPilot"))
        ) {
...
async function streamEvents(controller: ReadableStreamDefaultController, args: ExecutionArgs) {
  ...
  try {
    formattedMessages = neopilotMessagesToLangChain(messages);
  } catch (e) {
  ...
  if (!retrievedAssistant) {
    ...
    console.error(`
      No agent found for the agent name specified in NeoPilot provider
  ...
  try {
    ...
    for await (let streamResponseChunk of streamResponse) {
      ...
      if (streamResponseChunk.event === "error") {
        ...
        if (errorData && typeof errorData === "object" && "error_details" in errorData) {
          ...
          const preservedError = new NeoPilotLowLevelError({
        ...
        throw new NeoPilotLowLevelError({
      ...
      shouldExit =
        ...
        (eventType === LangGraphEventTypes.OnCustomEvent &&
          chunkData.name === CustomEventNames.NeoPilotExit);

      const emitIntermediateState = metadata["neopilot:emit-intermediate-state"];
      const manuallyEmitIntermediateState =
        eventType === LangGraphEventTypes.OnCustomEvent &&
        chunkData.name === CustomEventNames.NeoPilotManuallyEmitIntermediateState;
    ...
    // Re-throw NeoPilot errors so they can be handled properly at higher levels
    if (
      e instanceof NeoPilotError ||
      e instanceof NeoPilotLowLevelError ||
      (e instanceof Error && e.name && e.name.includes("NeoPilot"))
    ) {
      throw e;
    }
...
function getStateSyncEvent({
  ...
  if (!includeMessages) {
    ...
    state = {
      ...state,
      messages: langchainMessagesToNeoPilot(state.messages || []),
    };
  }
  ...
  return (
    JSON.stringify({
      event: LangGraphEventTypes.OnNeoPilotStateSync,
      thread_id: threadId,
      run_id: runId,
...
export function langchainMessagesToNeoPilot(messages: any[]): any[] {
  const result: any[] = [];
...
function neopilotMessagesToLangChain(messages: Message[]): LangGraphPlatformMessage[] {
...
function getSchemaKeys(graphSchema: GraphSchema): SchemaKeys {
  const CONSTANT_KEYS = ["messages", "neopilot"];
...
function emitInterrupt(interruptValue: any, emit: (data: string) => void) {
  if (typeof interruptValue != "string" && "__neopilot_interrupt_value__" in interruptValue) {
    const evValue = interruptValue.__neopilot_interrupt_value__;
    emit(
      JSON.stringify({
        event: LangGraphEventTypes.OnNeoPilotInterrupt,
        data: {
          value: typeof evValue === "string" ? evValue : JSON.stringify(evValue),
          messages: langchainMessagesToNeoPilot(interruptValue.__neopilot_messages__),
        },
      }) + "\n",
    );
  } else {
