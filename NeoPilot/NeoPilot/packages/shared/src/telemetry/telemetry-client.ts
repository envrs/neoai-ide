...
export class TelemetryClient {
  ...
  constructor({
    ...
    this.telemetryDisabled =
      telemetryDisabled ||
      (process.env as any).NEOPILOT_TELEMETRY_DISABLED === "true" ||
      (process.env as any).NEOPILOT_TELEMETRY_DISABLED === "1" ||
    ...
    const writeKey = process.env.NEOPILOT_SEGMENT_WRITE_KEY || "n7XAZtQCGS2v1vvBy3LgBCv2h3Y8whja";
    ...
    this.setGlobalProperties({
      "neopilot.package.name": packageName,
      "neopilot.package.version": packageVersion,
    });
  }
  ...
  private setSampleRate(sampleRate: number | undefined) {
    ...
    // eslint-disable-next-line
    if (process.env.NEOPILOT_TELEMETRY_SAMPLE_RATE) {
      // eslint-disable-next-line
      _sampleRate = parseFloat(process.env.NEOPILOT_TELEMETRY_SAMPLE_RATE);
    }
