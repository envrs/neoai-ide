/**
 * Console styling utilities for NeoPilot branded messages
...
/**
 * Styled console message for NeoPilot Platform promotion
 * Displays a beautiful, branded advertisement in the console
 */
export function logNeoPilotPlatformMessage() {
  console.log(
    `%cNeoPilot Warning%c

useCopilotChatHeadless_c provides full compatibility with NeoPilot's newly released Headless UI feature set. To enable this premium feature, add your public license key, available for free at:

%chttps://cloud.neopilot.ai%c
...
%chttps://docs.neopilot.ai/premium%c`,
...
export function publicApiKeyRequired(feature: string) {
  console.log(
    `
%cNeoPilot Warning%c \n
In order to use ${feature}, you need to add your NeoPilot API key, available for free at https://cloud.neopilot.ai.
    `.trim(),
    ConsoleStyles.header,
    ConsoleStyles.body,
  );
 ...
 * ```typescript
 * logStyled(
 *   '%cNeoPilot%c Welcome to the platform!',
...
export const styledConsole = {
  ...
  /** Log the NeoPilot platform promotion */
  logNeoPilotPlatformMessage: logNeoPilotPlatformMessage,
