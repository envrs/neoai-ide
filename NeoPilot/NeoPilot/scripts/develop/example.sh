...
if [ "$1" == "--help" ]; then
  ...
  echo "  - backend: The backend to use (readyapi or langgraph-platform, default: readyapi)"
  echo ""
...
if [[ "$backend" != "readyapi" && "$backend" != "langgraph-platform" ]]; then
  echo "Invalid backend: $backend. Must be 'readyapi' or 'langgraph-platform'."
  exit 1
fi
...
rm -rf .next
pnpm i
pnpm link --global @neopilot/react-ui @neopilot/react-core @neopilot/runtime-client-gql \
  @neopilot/shared @neopilot/runtime @neopilot/sdk-js
...
# Add sdk-python as an editable package only if the backend is readyapi
if [[ "$backend" == "readyapi" ]]; then
...
neopilot_prompt="\x1b[1;32m[NeoPilot]\x1b[0m"
...
parallel --ungroup ::: \
  "cd $root_dir && exec > >(sed 's/^/$neopilot_prompt /') 2>&1 && turbo run dev" \
