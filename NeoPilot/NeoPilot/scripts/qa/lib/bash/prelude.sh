...
if [ -n "$prerelease_tag" ]; then
  echo "Fetching pre-release NeoPilot packages..."
  packages=$(get_latest_neopilot_prerelase_versions "$prerelease_tag")
  echo "Pre-release NeoPilot packages: $packages"
fi

if [ -z "$packages" ]; then
  echo "No pre-release NeoPilot packages provided."
