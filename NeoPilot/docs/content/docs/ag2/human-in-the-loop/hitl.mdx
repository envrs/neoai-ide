...
<video
  src="https://cdn.neopilot.ai/docs/neopilot/images/coagents/node-hitl.mp4"
...
In this guide, we'll enhance our travel agent from the [Quick Start](/ag2/quickstart/ag2) guide with HITL functionality. The best part? We only need to modify the AG2 backend, no changes to the NeoPilot frontend are required!

You can find the complete code for this example in the [AG2-NeoPilot starter](https://github.com/ag2ai/ag2-neopilot-starter/blob/main/agent-py/hitl_workflow.py) repository.
...
sequenceDiagram
    participant User
    participant NeoPilot as NeoPilot UI
    participant TravelAgent as Travel Agent (AG2)
    participant Functions as Backend Functions

    User->>NeoPilot: Provides member ID
    NeoPilot->>TravelAgent: Forwards member ID
    TravelAgent->>NeoPilot: Requests to call lookup_member()
    NeoPilot->>User: Shows approval request
    User->>NeoPilot: Approves function call
    NeoPilot->>TravelAgent: Approval granted
    TravelAgent->>Functions: Calls lookup_member()
    Functions->>TravelAgent: Returns member data
    TravelAgent->>NeoPilot: Requests travel details
    User->>NeoPilot: Provides travel details
    NeoPilot->>TravelAgent: Forwards travel details
    TravelAgent->>NeoPilot: Requests to call create_itinerary()
    NeoPilot->>User: Shows approval request
    User->>NeoPilot: Approves function call
    NeoPilot->>TravelAgent: Approval granted
    TravelAgent->>Functions: Calls create_itinerary()
    Functions->>TravelAgent: Returns personalized itinerary
    TravelAgent->>NeoPilot: Displays itinerary to user
    NeoPilot->>User: Shows final itinerary
```
...
- **No NeoPilot changes needed**: The HITL implementation only requires changes to the AG2 backend. Your existing NeoPilot frontend will automatically handle the approval UI without any modifications.

Let's start by importing the necessary libraries and setting up the basic structure. You can either create the `agent-py/hitl_workflow.py` file from scratch if you wish to follow along, or you can copy the code from the [AG2-NeoPilot starter](https://github.com/ag2ai/ag2-neopilot-starter/blob/main/agent-py/hitl_workflow.py) repository.
...
# Set up ReadyAPI with the AWP adapter
...
```
- In a separate terminal, start the NeoPilot frontend:
```bash
cd ui
pnpm run dev
```
...
<img src="https://cdn.neopilot.ai/docs/neopilot/images/coagents/ag2/hitl-1.png" className="rounded-lg shadow-xl"/>
<img src="https://cdn.neopilot.ai/docs/neopilot/images/coagents/ag2/hitl-2.png" className="rounded-lg shadow-xl"/>
