...
In this guide, we'll teach you how to link the NeoPilot packages to your own project to test your changes. This way, you can run `turbo run dev` in the NeoPilot monorepo, and see changes in your own project immediately.

## Global Package Linking With `pnpm`

We will use pnpm's [global linking feature](https://pnpm.io/cli/link#pnpm-link---global) to link the NeoPilot packages to your own project.

<Steps>
  <Step>
  ### In The NeoPilot Monorepo
  Assuming you followed all steps in the contribution guide, you should have everything set up properly and ready to go.

  Navigate to the NeoPilot monorepo and run the following command to link the packages globally:
  ...
  ```bash
  # For example, to link the @neopilot/react-core package:
  pnpm link --global @neopilot/react-core
  ```
  ...
  <Step>
  ### Unlinking
  Once you are done, you can undo the global linking by running the following command in the NeoPilot monorepo:
