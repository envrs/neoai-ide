...
We use anonymous telemetry (metadata-only) to learn how to improve NeoPilot.
...
* We **do not collect any application data** flowing through your system, only NeoPilot metadata
...
In CopilotRuntime, simply set `NEOPILOT_TELEMETRY_DISABLED=true`. We also respect [Do Not Track (DNT)](https://consoledonottrack.com/).
...
The default sample rate is `0.05` (5%). You can adjust it by setting the `NEOPILOT_TELEMETRY_SAMPLE_RATE` to any value between 0 and 1.

## Get in touch

If you have any questions or concerns, please reach out at [hello@neopilot.ai](mailto:hello@neopilot.ai).