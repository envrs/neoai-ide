...
<Callout type="info">
  ...
  starter](https://github.com/neopilot-ai/neopilot/tree/main/examples/coagents-starter-crewai-flows)
...
NeoPilot lets you to add custom UI to take user input and then pass it back to the agent upon completion.
...
<Steps>
    <Step>
        ...
        You'll need to run your agent and connect it to NeoPilot before proceeding. If you haven't done so already,
        you can follow the instructions in the [Getting Started](/crewai-flows/quickstart/crewai) guide.

        If you don't already have an agent, you can use the [coagent starter](https://github.com/neopilot-ai/neoai-ide/tree/main/examples/coagents-starter-crewai-flows) as a starting point
    ...
    - The agent's state inherits from `NeoPilotState` to bring in the NeoPilot actions.
    - NeoPilot's actions are bound to the model as tools.
      ...
      <Tab value="Python">
        ...
        class SampleAgentFlow(Flow[AgentState]):
            ...
            async def check_for_user_feedback(self):
                ...
                # Expecting the result of a NeoPilot tool call (SEND/CANCEL)
            ...
            async def chat(self):
                ...
                response = await neopilot_stream(
                    completion(
                        ...
                        tools=self.state["neopilot"]["actions"],
                        stream=True
                    )
                )
