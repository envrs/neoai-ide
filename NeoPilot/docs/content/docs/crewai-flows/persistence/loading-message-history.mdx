...
CrewAI Flows supports threads, a way to group messages together and maintain a continuous chat history across sessions. NeoPilot provides mechanisms to ensure conversation state is properly persisted between the frontend and backend.
...
See the [sample agent implementation](https://github.com/neopilot-ai/neoai-ide/blob/main/examples/coagents-starter-crewai-flows/agent-py/sample_agent/agent.py#L291)
...
To load an existing thread in NeoPilot, set the `threadId` property on `<NeoPilot>`:
...
You can make the `threadId` dynamic. Once set, NeoPilot will load previous messages for that thread.
...
NeoPilot provides the current `threadId` and a `setThreadId` function from the `useCopilotContext` hook:
