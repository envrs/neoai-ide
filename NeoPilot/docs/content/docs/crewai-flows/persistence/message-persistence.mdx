...
For a concrete example of how a custom persistence class like `InMemoryFlowPersistence` can be implemented and used with the `@persist` decorator, see the [sample agent implementation](https://github.com/neopilot-ai/neoai-ide/blob/main/examples/coagents-starter-crewai-flows/agent-py/sample_agent/agent.py).
