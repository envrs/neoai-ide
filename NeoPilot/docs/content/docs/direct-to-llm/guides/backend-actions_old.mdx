...
The starting point for backend actions is the `CopilotRuntime` you setup during quickstart (the NeoPilot backend endpoint).
...
<Tabs items={['TypeScript / Node.js', 'LangChain JS', 'LangServe', 'Python + Remote Actions']}>
  ...
  **Note that `actions` is not merely an array of actions, but rather a _generator_ of actions.**
  ...
  as well as custom properties you can pass from the frontend.
    ...
    ```tsx title="/api/neopilot/route.ts"
  ...
  Simply return chain.stream and the results will be piped back to the Copilot.
    ...
    {/* LangServe tab */}
  <Tab value="LangServe">
    LangServe allows you to integrate LangChain chains hosted as separate services into your NeoPilot application.
  ...
  <Tab value="Python + Remote Actions">
    Remote actions allow you to integrate external services or APIs into your NeoPilot application.
    Any service that conforms to the Open-Source NeoPilot backend protocol can be served here.
