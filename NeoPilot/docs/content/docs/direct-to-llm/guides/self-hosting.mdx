...
The Copilot Runtime is the back-end component of NeoPilot, handling the communication with LLM, message history, state and more.

You may choose to self-host the Copilot Runtime, or [use Copilot Cloud](https://cloud.neopilot.ai) (recommended).

<Frame>
  <div className="w-full pb-4">
  ```mermaid
  sequenceDiagram
    participant core as @neopilot/react-core
...
<Step>
  ### Step 3: Configure the `<NeoPilot>` Provider
  <SelfHostingCopilotRuntimeConfigureNeoPilotProvider components={props.components} />
</Step>
