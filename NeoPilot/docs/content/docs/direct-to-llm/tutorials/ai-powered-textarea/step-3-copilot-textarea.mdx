...
Head over to the [`/components/Reply.tsx`](https://github.com/neopilot-ai/example-textarea/blob/base-start-here/components/Reply.tsx) file.
...
import { CopilotTextarea } from "@neopilot/react-textarea"; // [!code highlight]
...
We have implemented the `<CopilotTextarea />` component, but there is an issue - the copilot assistant is not aware of the email thread. In the next step, we'll make NeoPilot aware of our email history.
