...
At this point, we have set up our NeoPilot provider and `<CopilotTextarea />`, and we already benefit from a great AI assistant. However, there is one last problem - the copilot assistant is not aware of the email thread. Let's fix that.

## Our App's State

Let's quickly review how our app's state works. Open up the [`lib/hooks/use-emails.tsx`](https://github.com/neopilot-ai/example-textarea/blob/base-start-here/lib/hooks/use-emails.tsx) file.
