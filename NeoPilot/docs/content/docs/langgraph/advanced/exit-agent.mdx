...
After your agent has finished a workflow, you'll usually want to explicitly end that loop by calling the NeoPilot exit method in your agent code.
...
In this example from [our email-sending app](https://github.com/neopilot-ai/neopilot/tree/main/examples/coagents-qa), the `send_email` node explicitly exits, then manually sends a response back to the user as a `ToolMessage`:
        ...
        <Tabs groupId="language_langgraph_agent" items={['Python', 'TypeScript']} default="Python" persist>
            <Tab value="Python">
                ...
                async def send_email_node(state: EmailAgentState, config: RunnableConfig):
                    ...
                    await neopilot_exit(config) # [!code highlight]
            ...
            <Tab value="TypeScript">
                ```typescript
                import { neopilotExit } from "@neopilot/sdk-js/langgraph";
                ...
                async function sendEmailNode(state: EmailAgentState, config: RunnableConfig): Promise<{ messages: any[] }> {
                    ...
                    await neopilotExit(config); // [!code highlight]
