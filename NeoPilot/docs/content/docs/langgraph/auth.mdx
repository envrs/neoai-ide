...
NeoPilot supports user authentication for LangGraph agents in two deployment modes:
...
sequenceDiagram
    ...
    participant NeoPilot
    participant LangGraphAgent
    participant Agent

    Frontend->>NeoPilot: authorization: "user-token"
    NeoPilot->>LangGraphAgent: Forward auth token
...
sdk = NeoPilotRemoteEndpoint(
    agents=lambda context: [
        LangGraphAgent(
            ...
            langgraph_config={
                "configurable": {
                    "neopilot_auth": context["properties"].get("authorization")
                }
            }
        )
    ],
...
async def my_agent_node(state: AgentState, config: RunnableConfig):
    ...
    auth_token = config["configurable"].get("neopilot_auth")
    if auth_token:
...
async def my_agent_node(state: AgentState, config: RunnableConfig):
    ...
    # Self-hosted mode
    elif "configurable" in config and "neopilot_auth" in config["configurable"]:
        auth_token = config["configurable"]["neopilot_auth"]
        if auth_token:
            user_info = validate_your_token(auth_token)
            user_id = user_info["user_id"]
            user_role = user_info.get("role")
...
- NeoPilot automatically adds the `Bearer ` prefix for LangGraph Platform
...
- **Self-hosted**: Check that `neopilot_auth` is properly passed in `langgraph_config`
