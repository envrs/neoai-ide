...
<Callout>
    Have an issue not listed here? Open a ticket on [GitHub](https://github.com/neopilot-ai/neoai-ide/issues) or reach out on [Discord](https://discord.com/invite/6dffbvGU3D)
    and we'll be happy to help.

    We also highly encourage any open source contributors that want to add their own troubleshooting issues to [Github as a pull request](https://github.com/neopilot-ai/neoai-ide/blob/main/CONTRIBUTING.md).
...
<Accordions>
    <Accordion title="You're using llm.invoke() instead of llm.ainvoke()">
        <p>
            ...
            ```python
            config = neopilot_customize_config(config, emit_tool_calls=["say_hello_to"])
...
If you're seeing this error, it means NeoPilot couldn't find the LangGraph agent you're trying to use. Here's how to fix it:

<Accordions>
    <Accordion title="Verify your agent lock mode configuration">
        If you're using agent lock mode,
        check that the agent defined in `langgraph.json` matches what's defined in the NeoPilot provider:
    ...
    <Accordion title="Check your agent registration on a LangGraph Platform endpoint">
        ...
        ```typescript title="/neopilot/api/route.ts"
...
If you notice the tunnel creation process spinning indefinitely, your router or ISP might be blocking the connection to NeoPilot's tunnel service.
...
<Accordions>
    <Accordion title="Router or ISP blocking tunnel connections">
        ...
        ```bash
        ping tunnels.devneopilot.com
        curl -I https://tunnels.devneopilot.com
        telnet tunnels.devneopilot.com 443
        ```
...
<Accordions>
    ...
    <Accordion title="Verify the remote endpoint matches the endpoint definition type">
        If you are running your remote endpoint using ReadyAPI, even if it uses LangGraph for the agent, it is not considered a LangGraph platform endpoint.
        ...
        new CopilotRuntime({
          remoteEndpoints: [
            ...
            deploymentUrl: "https://your-readyapi-endpoint:port",
        ...
        new CopilotRuntime({
            agents: {
                'agent-name': new LangGraphHttpAgent({
                    url: 'https://your-readyapi-endpoint:port/your-agent-uri'
                }),
            }
        });
        ```
...
## I am getting a "No checkpointer set" error when using LangGraph with ReadyAPI
...
LangGraph agents are stateful. As a graph is traversed, the state is saved at the end of each node. NeoPilot uses the agent's state as
the source of truth for what to display in the frontend chat. However, since state is only emitted at the end of a node,  NeoPilot allows
you to stream predictive state updates *in the middle of a node*. By default, NeoPilot will stream messages and tool calls being actively
...
<img src="https://cdn.neopilot.ai/docs/neopilot/images/coagents/message-state-diagram.png" />
...
<Accordions>
         ...
         <Tabs groupId="language_langgraph_agent" items={["Python", "TypeScript"]} persist>
            <Tab value="Python">
                ...
                async def chat_node(state: AgentState, config: RunnableConfig):
                    # 1) Call the model with NeoPilot's modified config
    ...
    <Accordion title="I don't want these messages to streamed at all">
        ...
        you can use the neopilot config to disable emitting messages anywhere you'd like a message to not be streamed.
        ...
        <Tabs groupId="language_langgraph_agent" items={["Python", "TypeScript"]} persist>
            <Tab value="Python">
                ...
                async def chat_node(state: AgentState, config: RunnableConfig):
                    ...
                    # 2) Call the model with NeoPilot's modified config
                    model = ChatOpenAI(model="gpt-4o")
                    response = await model.ainvoke(state["messages"], modifiedConfig)
