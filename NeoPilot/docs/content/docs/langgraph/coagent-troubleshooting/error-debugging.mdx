...
description: "Learn how to debug errors in NeoPilot with dev console and set up error observability for monitoring services."
icon: "lucide/TriangleAlert"
---

NeoPilot provides comprehensive error handling capabilities to help you debug issues and monitor your application's behavior. Whether you're developing locally or running in production, NeoPilot offers tools to capture, understand, and resolve errors effectively.
...
<Callout type="info">
  **Need a publicApiKey?** Go to
  [https://cloud.neopilot.ai](https://cloud.neopilot.ai) and get one for
  free!
</Callout>
...
```tsx
<NeoPilot runtimeUrl="<your-runtime-url>" showDevConsole={true}>
  {/* Your app */}
</NeoPilot>
```
...
```tsx
import { CopilotErrorEvent } from "@neopilot/shared";
...
- Requires `publicApiKey` from [Copilot Cloud](https://cloud.neopilot.ai)
- Part of NeoPilot's enterprise observability offering
...
4. **Use it in your NeoPilot provider**:
   ```tsx
   <NeoPilot publicApiKey={process.env.NEXT_PUBLIC_NEOPILOT_API_KEY}>
     {/* Your app */}
   </NeoPilot>
   ```
...
- Learn about [Copilot Cloud features](https://cloud.neopilot.ai)
- Explore the [NeoPilot reference documentation](/reference/components/NeoPilot)
