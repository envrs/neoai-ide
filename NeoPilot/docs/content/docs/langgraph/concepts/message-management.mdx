...
Message management in CoAgents operates with NeoPilot as the "ground truth" for the full chat session. 
When an CoAgent session begins, it receives the existing NeoPilot chat history to maintain conversational
...
async def ask_name_node(state: GreetAgentState, config: RunnableConfig):
    ...
    await neopilot_emit_message(config, content)
...
## Message Flow
Messages flow between NeoPilot and LangGraph in a specific way:

- All messages from LangGraph are forwarded to NeoPilot
- On a fresh agent invocation, the full NeoPilot chat history is provided to the LangGraph agent as its pre-existing chat history.

When a CoAgent completes its execution, its relevant messages become part of NeoPilot's persistent chat history. This allows for all future agent invocations to get context from the full chat history.
