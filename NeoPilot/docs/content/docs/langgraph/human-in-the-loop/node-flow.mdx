...
<Callout type="error">
    The usage of node based interrupt is [now discouraged](https://langchain-ai.github.io/langgraph/concepts/v0-human-in-the-loop/) by both LangGraph and NeoPilot.
    As of LangGraph 0.2.57, the recommended way to set breakpoints is using [the interrupt function](https://docs.neopilot.ai/coagents/human-in-the-loop/interrupt-flow) as it simplifies human-in-the-loop patterns.
</Callout>

<video src="https://cdn.neopilot.ai/docs/neopilot/images/coagents/node-hitl.mp4" className="rounded-lg shadow-xl" loop playsInline controls autoPlay muted />

<Callout type="info">
  Pictured above is the [coagent starter](https://github.com/neopilot-ai/neopilot/tree/main/examples/coagents-starter) with
...
NeoPilot allows you to add custom UI to take user input and then pass it back to the agent upon completion.
...
<Steps>
    ...
    <Step>
        ...
        import { useHumanInTheLoop } from "@neopilot/react-core"
    ...
    - The agent's state inherits from `NeoPilotState` to bring in the NeoPilot tools.
    - NeoPilot's tools are binded to the model as tools.
    ...
    <Tabs groupId="language_langgraph_agent" items={["Python", "TypeScript"]} persist>
      <Tab value="Python">
        ...
        # 1. Define our agent's state and inherit from NeoPilotState, this brings in the NeoPilot tools
        class AgentState(NeoPilotState): # [!code highlight]
        ...
        async def chat_node(state: AgentState, config: RunnableConfig) -> Command[Literal["user_feedback_node", "__end__"]]:
            # 2.1 Define the model and bind NeoPilot's tools as tools
            model = ChatOpenAI(model="gpt-4o")
            model_with_tools = model.bind_tools([*state.get("neopilot", {}).get("actions", [])]) # [!code highlight]
        ...
        #    where NeoPilot's renderAndWaitForResponse provide the user's response.
      ...
      <Tab value="TypeScript">
        ...
        // // 1. Import necessary helpers for NeoPilot tools
        import { convertActionsToDynamicStructuredTools } from "@neopilot/sdk-js/langgraph";
        import { NeoPilotStateAnnotation } from "@neopilot/sdk-js/langgraph";

        // 2. Define graph state, inherit from NeoPilotState to bring in NeoPilot tools
        //    and messages.
        ...
        async function chatNode(state: AgentState, config: RunnableConfig) {
          ...
          // 3.2 Bind the tools to the model, include NeoPilot tools. This allows
          ...
          if (response.tool_calls?.length) {
            ...
            //       execution, giving NeoPilot a chance to get feedback from the user.
            //       
            //       The "writeEssay" tool is a NeoPilot tool and is binded in step 3.2.
        ...
        //    execution the graph be interrupted, waiting for NeoPilot to get feedback
        //    from the user.
