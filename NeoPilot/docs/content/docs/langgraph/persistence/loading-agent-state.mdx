...
When setting the `threadId` property in NeoPilot, i.e:
...
```tsx
<NeoPilot threadId="2140b272-7180-410d-9526-f66210918b13">
  <YourApp />
</NeoPilot>
```

NeoPilot will restore the complete state of the thread, including the messages, from the database. (See [Message Persistence](/coagents/persistence/message-persistence) for more details.)
...
To learn more about persistence and state in NeoPilot, see:
