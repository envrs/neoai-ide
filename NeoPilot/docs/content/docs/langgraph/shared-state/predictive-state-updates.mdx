...
<IframeSwitcher
  ...
  exampleUrl="https://feature-viewer.neopilot.ai/langgraph/feature/predictive_state_updates?sidebar=false&chatDefaultOpen=false"
  codeUrl="https://feature-viewer.neopilot.ai/langgraph/feature/predictive_state_updates?view=code&sidebar=false&codeLayout=tabs"
  exampleLabel="Demo"
  codeLabel="Code"
  height="700px"
/>
...
<Callout type="info">
  This example demonstrates predictive state updates in the [NeoPilot Feature Viewer](https://feature-viewer.neopilot.ai/langgraph/feature/predictive_state_updates).
</Callout>
...
NeoPilot enables this through its concept of **_predictive state updates_**.
...
<Steps>
        ...
        >
            ...
            >
                ...
                <Tabs groupId="language_langgraph_agent" items={['Python', 'TypeScript']} default="Python" persist>
                    <Tab value="Python">
                        ```python title="agent.py"
                        from neopilot.langgraph import neopilot_emit_state # [!code highlight]
                        # ...
                        async def chat_node(state: AgentState, config: RunnableConfig) -> Command[Literal["cpk_action_node", "tool_node", "__end__"]]:
                            ...
                            for step in steps:
                                ...
                                await neopilot_emit_state(config, state) # [!code highlight]
                    ...
                    <Tab value="TypeScript">
                        ```ts title="agent-js/src/agent.ts"
                        import { neopilotEmitState } from "@neopilot/sdk-js/langgraph"; // [!code highlight]
                        // ...
                        async function chat_node(state: AgentState, config: RunnableConfig) {
                            ...
                            for (const step of steps) {
                                ...
                                neopilotEmitState(config, state);
            ...
            <TailoredContentOption
                ...
                icon={<FaWrench />}
            >
                For long-running tasks, you can configure NeoPilot to automatically predict state updates when specific tool calls are made. In this example, we'll configure NeoPilot to predict state updates whenever the LLM calls the step progress tool.
                <Tabs groupId="language_langgraph_agent" items={['Python', 'TypeScript']} default="Python" persist>
                    <Tab value="Python">
                        ```python
                        ...
                        async def frontend_actions_node(state: AgentState, config: RunnableConfig):
                            # Configure NeoPilot to treat step progress tool calls as predictive of the final state
                            ...
                            # Call the model with NeoPilot's modified config
    ...
    <Step>
        ...
        <video src="https://cdn.neopilot.ai/docs/neopilot/images/coagents/intermediate-state-render.mp4" className="rounded-lg shadow-xl" loop playsInline controls autoPlay muted />
        <Callout>
