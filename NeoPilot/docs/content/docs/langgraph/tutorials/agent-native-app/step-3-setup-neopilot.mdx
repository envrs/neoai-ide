...
Now that we have both our application and agent running, let's connect them using NeoPilot. The necessary dependencies are already installed in the `frontend` directory:

- `@neopilot/react-core`: Core NeoPilot functionality and hooks
- `@neopilot/react-ui`: Pre-built UI components for chat interfaces
...
<Accordion title="Install command (optional)">
  **The package versions in this tutorial are pinned, so updating the dependencies could break the tutorial.**
  ...
  ```npm
  @neopilot/react-core @neopilot/react-ui
  ```
</Accordion>
</Accordions>
...
Create a [Copilot Cloud account](https://cloud.neopilot.ai/sign-in) to get started. This provides a production-ready proxy to your LLMs.
...
<ImageZoom src="https://cdn.neopilot.ai/docs/neopilot/images/coagents/tutorials/ai-travel-app/cpkCloudSetup.png" alt="CopilotCloud API Key" height={1000} width={1000} />
...
<Step>
### Add the NeoPilot provider
Wrap your application with the NeoPilot provider:
...
<Callout>
  The `Chat` component will serve as a wrapper around the NeoPilot `CopilotChat` component. This is to help simplify
...
<Frame>
  <ImageZoom src="https://cdn.neopilot.ai/docs/neopilot/images/coagents/tutorials/research-ana/fe-step-3-finish.png" alt="step-3-finish" height={1000} width={1000} />
</Frame>
