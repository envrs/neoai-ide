...
For this guide, we're going to start using the NeoPilot SDK to emit the state of the agent manually. This is because
in LangGraph, state is only updated when a node change occurs (i.e, an edge is traversed). 

As such, in-progress work is not emitted to the user by default. However, we can manually emit the state using the `neopilot_emit_state`
...
<Step>
## NeoPilot Python SDK
This tutorial already comes with the NeoPilot Python SDK installed. This allows us to utilize various NeoPilot specific
...
<Step>
## Manually emit state
The research ANA emits state in a variety of places. For the sake of simplicity, we'll be adding the `neopilot_emit_state` function
...
Each of these files will write their progress to the `logs` field of the agent's state. Directly after that, we call `neopilot_emit_state` to emit the state to the frontend.

For example, in the `tool_node` we update some state based on the tool result and then use `neopilot_emit_state` to emit the state to the frontend.

```python title="agent/graph.py"
# ...
from neopilot.langchain import neopilot_emit_state # [!code ++]
#...
...
async def tool_node(self, state: ResearchState, config: RunnableConfig) -> Command[Literal["process_feedback_node", "call_model_node"]]:
        ...
        for tool_call in state["messages"][-1].tool_calls:
            ...
            await neopilot_emit_state(config, tool_state) # [!code ++]
...
you want to emit state to the frontend, you can do so by calling `neopilot_emit_state`.
...
import { useCoAgentStateRender, useLangGraphInterrupt } from "@neopilot/react-core"; // [!code ++]
...
<Frame className="mt-0">
  <ImageZoom className="mt-0" src="https://cdn.neopilot.ai/docs/neopilot/images/coagents/tutorials/research-ana/fe-step-7-finish.png" alt="CopilotCloud API Key" height={1000} width={1000} />
</Frame>

To recap, we did the following:
- Learned about how to emit state whenever we want with `neopilot_emit_state`.
