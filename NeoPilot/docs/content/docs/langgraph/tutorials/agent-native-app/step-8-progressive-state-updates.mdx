...
For this, we'll be using the `neopilot_customize_config`.

<Callout>
The `neopilot_customize_config` function will be used in the next step to render the agent's progress in the chat.
</Callout>

## Create a neopilot_customize_config and emit intermediate state
NeoPilot allows you to customize the configuration of how your frontend and agent interact with each other. In this case, we want to
...
from neopilot.langchain import neopilot_customize_config, neopilot_emit_state // [!code ++]
...
<video src="https://cdn.neopilot.ai/docs/neopilot/images/coagents/tutorials/research-ana/fe-step-8-finish.mp4" className="rounded-lg shadow-xl" loop playsInline controls autoPlay muted />

To recap, we did the following:
- Learned about the `neopilot_customize_config` function.
- Used the config to emit the state of the agent predictively

You've successfully created a CoAgent using the core concepts of NeoPilot. In the next step, we'll wrap up this tutorial by recapping the entire process
