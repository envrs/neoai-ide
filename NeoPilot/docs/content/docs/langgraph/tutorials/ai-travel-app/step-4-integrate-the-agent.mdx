...
Let's quickly review how the app's state works. Open up the [`lib/hooks/use-trips.tsx`](https://github.com/neopilot-ai/neoai-ide/blob/coagents-travel-tutorial-start/examples/coagents-travel/ui/lib/hooks/use-trips.tsx) file.
...
To integrate the agent into this state, we're going to need to register an agent with NeoPilot and then use the `useCoAgent` hook to connect the two.
...
We're going to use the NeoPilot CLI to setup a tunnel between our locally running LangGraph agent and Copilot Cloud. You'll 
...
<Frame>
  <img src="https://cdn.neopilot.ai/docs/neopilot/images/coagents/tutorials/ai-travel-app/lgs-endpoint.png" alt="LangGraph Studio Endpoint" />
</Frame>
...
In our previously setup `/api/neopilot` route, we're going to add the following.

```tsx title="ui/app/api/neopilot/route.ts"
// ...

import { CopilotRuntime, LangGraphAgent } from '@neopilot/runtime'; // [!code ++]
...
This allows NeoPilot to know where to send requests to when the agent is called.
...
This will ensure that every request is sent to the `travel` agent. The `travel` name is defined in the [agents/langgraph.json](https://github.com/neopilot-ai/neoai-ide/blob/main/examples/coagents-travel/agent/langgraph.json)
