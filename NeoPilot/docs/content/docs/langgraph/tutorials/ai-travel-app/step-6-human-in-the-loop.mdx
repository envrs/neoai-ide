...
<Frame>
    <img src="https://cdn.neopilot.ai/docs/neopilot/images/coagents/coagents-hitl-infographic.png" alt="Human in the loop" />
</Frame>
...
```python title="agent/travel/trips.py"
from neopilot.langchain import neopilot_emit_message # [!code ++]
...
async def perform_trips_node(state: AgentState, config: RunnableConfig):
    ...
    if tool_message.content == "CANCEL":
      await neopilot_emit_message(config, "Cancelled operation of trip.")
      return state
...
# ...
from neopilot.langchain import neopilot_customize_config # [!code ++]
...
handling to NeoPilot. If, for example, `search_for_places` was called here then it would break the state of tool calls.
</Callout>
...
import { useCoAgent, useCoAgentStateRender, useCopilotAction } from "@neopilot/react-core"; // [!code ++]
// ...
