...
- **Frontend**: Pass tokens via `<NeoPilot headers={{ Authorization: token }}>`
...
<Tabs groupId="language_microsoft-agent-framework_agent" items={['.NET', 'Python']} persist>
  ...
  <Tab value="Python">
    ...
    import os
    from readyapi import ReadyAPI, HTTPException, Request, status
    ...
    app = ReadyAPI(title="NeoPilot + Microsoft Agent Framework (Python)")
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
    ...
    add_agent_framework_readyapi_endpoint(app=app, agent=my_agent, path="/")
    ```
  </Tab>
</Tabs>
...
**Token not reaching server**: Verify the `Authorization` header is set in `<NeoPilot>` and forwarded through any proxies.
