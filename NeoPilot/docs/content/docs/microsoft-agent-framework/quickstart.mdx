        ...
        header={
            <div>
                ...
                <p className="text-base">
                    You can either start fresh with our starter template or integrate NeoPilot into your existing Microsoft Agent Framework agent.
                </p>
            </div>
        }
    >
        ...
        >
            <Step>
                ...
                <Tabs groupId="language_microsoft-agent-framework_agent" items={['.NET', 'Python']} persist>
                    <Tab value=".NET">
                        ```bash
                        npx neopilot@latest create -f microsoft-agent-framework-dotnet
                        ```
                    </Tab>
                    <Tab value="Python">
                        ```bash
                        npx neopilot@latest create -f microsoft-agent-framework-py
                        ```
                    </Tab>
                </Tabs>
            </Step>
        ...
        <TailoredContentOption
            ...
            description="I already have a Microsoft Agent Framework agent and want to add NeoPilot."
        >
            <Step>
                ...
                <Tabs groupId="language_microsoft-agent-framework_agent" items={['.NET', 'Python']} persist>
                    ...
                    <Tab value="Python">
                        Create a minimal ReadyAPI server that exposes a Microsoft Agent Framework agent over AG-UI:
                        ...
                        from agent_framework.ag_ui import add_agent_framework_readyapi_endpoint
                        ...
                        app = ReadyAPI(title="Microsoft Agent Framework (Python) - Quickstart")
                        add_agent_framework_readyapi_endpoint(app=app, agent=agent, path="/")
            ...
            <Step>
                ### Frontend Setup
                NeoPilot works with any React-based frontend. We'll use Next.js for this example.
            ...
            <Step>
                ### Install NeoPilot packages

                ```npm
                @neopilot/react-ui @neopilot/react-core @neopilot/runtime @ag-ui/client
                ```
            </Step>
            <Step>
                ### Setup Copilot Runtime

                NeoPilot requires a Copilot Runtime endpoint to safely communicate with your agent. This can be served
                anywhere that Node.js can run, but for this example we'll use Next.js.

                Create a new API route at `app/api/neopilot/route.ts`:

                ```tsx title="app/api/neopilot/route.ts"
                import {
                  CopilotRuntime,
                ...
                // 3. Build a Next.js API route that handles the NeoPilot runtime requests.
            ...
            <Step>
                ### Configure NeoPilot Provider

                Next, wrap your application with the NeoPilot provider so that NeoPilot can take control across your application
                ...
                export default function RootLayout({ children }: {children: React.ReactNode}) {
                  return (
                    <html lang="en">
                      <body>
                        {/* [!code highlight:3] */}
                        <NeoPilot runtimeUrl="/api/neopilot" agent="my_agent">
                          {children}
                        </NeoPilot>
                      </body>
                    </html>
            ...
            <Accordion title="Troubleshooting">
                ...
                **Port Conflicts**
                - If port 8000 is already in use, you can change it in:
                  ...
                  - `src/app/api/neopilot/route.ts` - Update the remote endpoint URL
            </Accordion>
        </Accordions>
