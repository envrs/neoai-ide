...
<Tabs groupId="lg-deployment-type" items={['Local (LangGraph Studio)', 'Self hosted (ReadyAPI)', 'LangGraph Platform']}>
  <Tab value="Local (LangGraph Studio)">
    ...
    <Accordions className="mb-4">
        ...
        <Accordion title='"No checkpointer set" error with langgraph dev'>
          ...
          - **With local ReadyAPI**: Don't set this flag; uses MemorySaver automatically
  ...
  <Tab value="Self hosted (ReadyAPI)">
    <Callout type="info">
    ...
    2. Next, use NeoPilot's ReadyAPI integration to serve your LangGraph agent.

    ```python title="server.py"
    import os
    from readyapi import ReadyAPI
    import uvicorn
    from neopilot import LangGraphAGUIAgent # [!code highlight]
    from ag_ui_langgraph import add_langgraph_readyapi_endpoint # [!code highlight]
    ...
    def main():
        ...
        uvicorn.run(
            "sample_agent.demo:app", # the path to your ReadyAPI file, replace this if its different
            host="0.0.0.0",
            port=port,
            reload=True,
        )
