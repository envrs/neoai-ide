## Overview
The NeoPilot MCP server equips AI coding agents with deep knowledge about NeoPilot's APIs, patterns, and best practices. When connected to your 
development environment, it enables AI assistants to:
- Provide expert guidance
- Generate accurate code
- Give your AI agents a user interface
- Help you implement NeoPilot features correctly
...
<Steps>
  ...
  <Step>
    ...
    Copy NeoPilot MCP's configuration and paste it under the mcpServers key in the mcp.json file.
...
<Steps>
  ...
  <Step>
    1. In the Name field, enter a memorable name for the NeoPilot connector, like `NeoPilot`
    2. In the URL field, enter the following:
    ```
    https://mcp.neopilot.ai/sse
    ```
  </Step>
  <Step>
    Click "Add"
...
<Steps>
  <Step>
    ...
    Use the Claude Code CLI to add the NeoPilot MCP server:

    ```bash
    claude mcp add --transport sse neopilot-mcp https://mcp.neopilot.ai/sse
    ```

    **Expected Output:**
    ```
    Added SSE MCP server neopilot-mcp with URL: https://mcp.neopilot.ai/sse to local config
  ...
  <Step>
    ...
    **Expected Output:**
    ...
    neopilot-mcp: https://mcp.neopilot.ai/sse (SSE) - ✓ Connected
    ```
  ...
  <Step>
    ...
    Once configured, the NeoPilot MCP server tools are automatically available when you interact with Claude Code for NeoPilot-related development tasks. The AI will intelligently use these tools when relevant to your queries.

    **What the MCP Server Provides:**
    - Expert guidance for NeoPilot development
    - Accurate code generation for NeoPilot features
    - Best practices and implementation patterns
    - Deep understanding of NeoPilot APIs

    **Management Commands:**
    ```bash
    # View server details
    claude mcp get neopilot-mcp

    # Remove server if needed
    claude mcp remove neopilot-mcp -s local
    ```
  </Step>
</Steps>
...
<Steps>
  ...
  <Step>
    ...
    You can add NeoPilot MCP in several ways:
          ...
          <Tab value="stdio Transport (Local)">
            ```json
            {
              "mcpServers": {
                "NeoPilot MCP": {
                  "command": "npx",
                  "args": ["mcp-remote", "https://mcp.neopilot.ai"]
                }
              }
            }
            ```
  ...
  <Step>
    ...
    Once configured, NeoPilot MCP tools will be available in Windsurf's Cascade AI assistant:

    - Open the Cascade panel (AI chat interface)
    - The MCP tools are automatically available to the AI
    - You can reference specific tools using `@NeoPilot MCP` in your conversations
    ...
    The AI will seamlessly integrate NeoPilot MCP functionality into your development workflow!
  </Step>
</Steps>
...
<Steps>
  ...
  <Step>
    ...
    <Tabs groupId="cline-setup" items={['Remote Server Setup', 'Configuration File Setup']} default="Remote Server Setup">
      <Tab value="Remote Server Setup">
        ...
        2. Enter a Server Name (e.g., "NeoPilot MCP")
  ...
  <Step>
    ...
    Once connected, Cline can automatically use the tools provided by NeoPilot MCP when you interact with the AI assistant. The MCP tools will be available without requiring manual selection - Cline's AI will intelligently choose which tools to use based on your requests.
  ...
  <Step>
    ...
    <Tabs groupId="copilot-config" items={['Workspace Configuration (Recommended)', 'User Settings Configuration', 'Command Palette']} default="Workspace Configuration (Recommended)">
      ...
      <Tab value="Command Palette">
        ...
        4. Enter the server URL: `https://mcp.neopilot.ai/sse`
        5. Provide a name for the server: `NeoPilot MCP`
      </Tab>
    </Tabs>
  </Step>
  <Step>
    ...
    4. Your NeoPilot MCP tools will be listed and can be used automatically
...
2. **Add Server** - Use the SSE URL: `https://mcp.neopilot.ai/sse`
