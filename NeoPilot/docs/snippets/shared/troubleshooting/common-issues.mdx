...
Welcome to the NeoPilot Troubleshooting Guide! Here, you can find answers to common issues
...
<Accordions>
    <Accordion title="Check your endpoint configuration">
        Verify your endpoint configuration in your NeoPilot setup:
    ...
    <Accordion title="localhost vs 127.0.0.1">
        ...
        http://localhost:3000/api/neopilot

        # Try this instead:
        http://127.0.0.1:3000/api/neopilot
        ```
...
## I am getting "NeoPilot's Remote Endpoint" not found error

If you're getting a "NeoPilot's Remote Endpoint not found" error, it usually means the server serving `/info` endpoint isn't accessible. Here's how to fix it:

<Accordions>
    <Accordion title="Check your ReadyAPI setup (if using python's ReadyAPI)">
        Make sure your ReadyAPI app has the NeoPilotSDK properly set up.<br/>
    ...
    <Accordion title="Test your endpoint">
        ...
        ```bash
        curl -v -d '{}' http://localhost:8000/neopilot/info
        ```
        ...
        * Connection #0 to host localhost left intact
        ...
        If you see a different response, check your ReadyAPI logs for errors.
    </Accordion>
