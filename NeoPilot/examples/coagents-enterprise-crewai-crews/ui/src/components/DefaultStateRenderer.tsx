...
const DefaultLoaderIcon: React.FC<ResponseRendererIconProps> = ({
  ...
  <svg
    className={`${className} neopilot-spinner`}
    width="16"
    height="16"
    viewBox="0 0 24 24"
    fill="none"
...
const DefaultSkeletonLoader: React.FC<SkeletonLoaderProps> = ({
  ...
  <div className={className || "neopilot-skeleton"}>
    <div className="neopilot-skeleton-header">
      <div className="neopilot-skeleton-title"></div>
      <div className="neopilot-skeleton-subtitle"></div>
    </div>
    <div className="neopilot-skeleton-content"></div>
  </div>
);
...
const DefaultStateItemRenderer: React.FC<StateItemRendererProps> = ({
  ...
  <div
    className={`${className || "neopilot-state-item"} ${
      isNewest ? "neopilot-state-item-newest" : ""
    }`}
  >
    <div className="neopilot-state-item-header">
      {"tool" in item ? item.tool : item.name}
    </div>

    {"thought" in item && item.thought && (
      <div className="neopilot-state-item-thought">{item.thought}</div>
    )}

    {"result" in item && item.result !== undefined && item.result !== null && (
      <div className="neopilot-state-item-result">
        {formatContent(item.result)}
      </div>
    )}

    {"description" in item && item.description && (
      <div className="neopilot-state-item-description">
        {item.description}
      </div>
    )}
  </div>
...
export const DefaultStateRenderer: React.FC<StateRendererProps> = ({
  ...
  labels,
  icons,
  className = "neopilot-state",
  contentClassName = "neopilot-state-content",
  itemClassName = "neopilot-state-item",
  maxHeight = "250px",
  ...
  if (!state) {
    return (
      <div className={className}>
        <DefaultLoaderIcon className="neopilot-loader" />
      </div>
    );
  }
  ...
  return (
    <div className={className}>
      {/* Header with toggle */}
      <div
        className="neopilot-state-header"
        onClick={() => setIsCollapsed(!isCollapsed)}
      >
        {isCollapsed ? (
          <ExpandIcon className="neopilot-icon" />
        ) : (
          <CollapseIcon className="neopilot-icon" />
        )}
        <div className="neopilot-state-label">
          {status === "inProgress" ? (
            <span className="neopilot-state-label-loading">
      ...
      {!isCollapsed && (
        ...
        >
          ...
          {items.length > 0 ? (
            ...
            // Empty state
            <div className="neopilot-state-empty">
