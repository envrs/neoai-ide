...
from neopilot.langgraph import (
  neopilot_customize_config, neopilot_exit, neopilot_emit_message, neopilot_interrupt
)
...
async def email_node(state: EmailAgentState, config: RunnableConfig):
    ...
    if sender_company is None:
        sender_company, new_messages = neopilot_interrupt(message='Ah, forgot to ask, which company are you working for?')
    ...
    cpk_actions = state.get("neopilot", {}).get("actions", [])
...
async def send_email_node(state: EmailAgentState, config: RunnableConfig):
    ...
    await neopilot_emit_message(config, text_message)
...
# This allows compatibility with both LangGraph API and NeoPilot
