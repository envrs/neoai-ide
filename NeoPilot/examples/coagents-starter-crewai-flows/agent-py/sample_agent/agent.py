...
from neopilot.crewai import neopilot_stream, NeoPilotState, neopilot_exit

class AgentState(NeoPilotState):
    """
    Here we define the state of the agent

    In this instance, we're inheriting from NeoPilotState, which will bring in
    the NeoPilotState fields. We're also adding a custom field, `language`,
    which will be used to set the language of the agent.
    """
    language: Literal["english", "spanish"] = "english"
    proverbs: List[str] = ["NeoPilot may be new, but its the best thing since sliced bread."]
...
class InMemoryFlowPersistence(FlowPersistence):
    ...
    def save_state(
        ...
        if "messages" in state_dict and state_dict["messages"]:
            ...
            # Convert any Message objects to a format that's compatible with crewai_flow_messages_to_neopilot
            serialized_messages = []
            for msg in state_dict["messages"]:
                ...
                # Ensure necessary fields are present for crewai_flow_messages_to_neopilot
                ...
                # Handle tool calls - ensure they're in the format expected by crewai_flow_messages_to_neopilot
        ...
        # Special handling for NeoPilotProperties
        if "neopilot" in state_dict:
            print(f"[SAVE DEBUG] Handling neopilot property of type {type(state_dict['neopilot']).__name__}")
            if hasattr(state_dict["neopilot"], "model_dump"):
                state_dict["neopilot"] = state_dict["neopilot"].model_dump()
            elif hasattr(state_dict["neopilot"], "dict"):
                state_dict["neopilot"] = state_dict["neopilot"].dict()
            elif not isinstance(state_dict["neopilot"], dict):
                # Convert to a basic dict
                actions = getattr(state_dict["neopilot"], "actions", [])
                ...
                state_dict["neopilot"] = {"actions": serialized_actions}
    ...
    def load_state(self, flow_uuid: str) -> Optional[Dict[str, Any]]:
        ...
        # Special handling for NeoPilotProperties
        if "neopilot" in state:
            if state["neopilot"] is None:
                # Replace with empty dict if None
                state["neopilot"] = {"actions": []}
            elif hasattr(state["neopilot"], "model_dump"):
                state["neopilot"] = state["neopilot"].model_dump()
            elif hasattr(state["neopilot"], "dict"):
                state["neopilot"] = state["neopilot"].dict()
            elif not isinstance(state["neopilot"], dict):
                # Convert to a basic dict if it's not already
                state["neopilot"] = {"actions": getattr(state["neopilot"], "actions", [])}
...
class SampleAgentFlow(Flow[AgentState]):
    """
    This is a sample flow that uses the NeoPilot framework to create a chat agent.
    """
    ...
    async def start_flow(self):
        ...
        # The NeoPilot SDK's execute_flow method automatically sets state['id'] = thread_id
    ...
    async def chat(self):
        ...
        - The model to use (and binds in NeoPilot actions and the tools defined above)
        - The system prompt
        ...
        if message.get("tool_calls"):
            ...
            #    is a NeoPilot action, we return the response to NeoPilot to handle
            if (tool_call_name in
                [action["function"]["name"] for action in self.state.neopilot.actions]):
