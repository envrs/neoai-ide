...
**Option 2: Using Poetry (Local ReadyAPI)**
...
The UI connects to your agent via the configuration in `ui/app/api/neopilot/route.ts`. The current setup depends on which agent and method you're using:
...
**If using Poetry/ReadyAPI (Option 2):**
...
- Run this command to connect your Copilot Cloud Tunnel to the LangGraph server `npx neopilot@latest dev --port 8123`
...
4. **Route configuration:** Make sure your `ui/app/api/neopilot/route.ts` matches your agent setup:
   - LangGraph dev server (port 8000) → Use `langGraphPlatformEndpoint`
   - ReadyAPI demo (port 8000) → Use basic endpoint with `/neopilot` path
