...
// 1. Import necessary helpers for NeoPilot actions
import { convertActionsToDynamicStructuredTools, NeoPilotStateAnnotation } from "@neopilot/sdk-js/langgraph";

// 2. Define our agent state, which includes NeoPilot state to
...
async function chat_node(state: AgentState, config: RunnableConfig) {
  ...
  // 6.2 Bind the tools to the model, include NeoPilot actions. This allows
  //     the model to call tools that are defined in NeoPilot by the frontend.
...
function shouldContinue({ messages, neopilot }: AgentState) {
  // 7.1 Get the last message from the state
  const lastMessage = messages[messages.length - 1] as AIMessage;

  // 7.2 If the LLM makes a tool call, then we route to the "tools" node
  if (lastMessage.tool_calls?.length) {
    const actions = neopilot?.actions;
    const toolCallName = lastMessage.tool_calls![0].name;

    // 7.3 Only route to the tool node if the tool call is not a NeoPilot action
