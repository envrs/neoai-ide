...
This hook is where most of the magic happens for NeoPilot (if you're looking for the rest, check out [app/page.tsx](../../app/page.tsx) and [api/neopilot/route.ts](../../app/api/neopilot/route.ts)). It handles interacting with the CoAgent's state and provides helper functions for the UI to interact with and update the CoAgent's state. It also provides intermediate streaming for human-in-the-loop components as well as progress tracking.
...
For more information on how to use `useCoAgent`, check out the [reading](https://docs.neopilot.ai/coagents/shared-state/in-app-agent-read) and [writing](https://docs.neopilot.ai/coagents/shared-state/in-app-agent-write) state docs.
...
```python
    ...
    # ...
    await neopilot_emit_state(config, state)
    # ...
```

With those lines of code, the `useCoAgentStateRender` hook will be able to detect the `neopilot_emit_state` calls and trigger our rendering logic in the chat `search_progress` state.
...
2. Emitting tool calls via the NeoPilot SDK.
3. Sending approval/rejection response back to the agent via our `useCopilotAction` hook.

This is gone into in more detail in our documentation on implementing [human in the loop](https://docs.neopilot.ai/coagents/human-in-the-loop/node-flow).
