...
1. Clone the repository:
   ...
   cd NeoPilot/examples/copilot-form-filling
   ```
...
3. Create a `.env` file in the project root and add your [Copilot Cloud Public API Key](https://cloud.neopilot.ai):
   ```
   NEXT_PUBLIC_COPILOT_PUBLIC_API_KEY=your_neopilot_api_key
   ```
...
This demo uses several key NeoPilot features:
  ...
  return (
    <html lang="en">
      <body className={`${geistSans.variable} ${geistMono.variable} antialiased`}>
        <NeoPilot publicApiKey={process.env.NEXT_PUBLIC_COPILOT_PUBLIC_API_KEY}>
          {children}
        </NeoPilot>
      </body>
    </html>
