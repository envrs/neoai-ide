...
export default class StructuredReporter implements Reporter {
    ...
    if (testCase.results.length === (test.retries ?? 0) + 1) {
      ...
      if (lastResult.status === "failed" || lastResult.status === "timedOut") {
        ...
        if (videoAttachment?.path) {
          ...
          const videoUrl = `https://us-east-1.console.aws.amazon.com/s3/object/neopilot-e2e-test-recordings?region=us-east-1&bucketType=general&prefix=${objectPath}`;
  ...
  async onEnd(result: FullResult) {
    ...
    const commitLink = `[${shortSha}](https://github.com/neopilot-ai/neoai-ide/commit/${commitSha})`;
