...
async function main() {
  ...
  const previewReleases = releaseList
    .filter((release) => release.namespace.startsWith("neopilot-examples"))
    .filter((release) => release.name === "next-openai")
    .filter((release) => release.namespace !== "neopilot-examples-main");
  ...
  for (const release of inactiveReleases) {
    const environment = release.namespace.replace("neopilot-examples-", "");
