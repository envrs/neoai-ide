...
from neopilot.langgraph import neopilot_customize_config, neopilot_emit_state, neopilot_interrupt
...
class AgentState(NeoPilotState):
    ...
    It inherits from NeoPilotState which provides the basic fields needed by NeoPilot.
    """
...
async def chat_node(state: Dict[str, Any], config: RunnableConfig):
    ...
    For every agent request, YOU MUST ALWAYS GENERATE 4 DIFFERENT TEST SUITES, each as a separate object in the array. Each test suite should be relevant to the context which is the NeoPilotReadables or PR provided by the user, and should have unique test cases and details. All the data which involves the user emails should be referred from the NeoPilotReadables.
    ...
    # Use NeoPilot's custom config functions to properly set up streaming for the steps state
