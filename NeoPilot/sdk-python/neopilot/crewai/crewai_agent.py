...
from neopilot.agent import Agent
from neopilot.types import Message
from neopilot.action import ActionDict
from neopilot.protocol import (
  emit_runtime_events,
  agent_state_message,
)
from neopilot.crewai.crewai_sdk import (
  neopilot_messages_to_crewai_flow,
  crewai_flow_messages_to_neopilot,
...
from neopilot.runloop import neopilot_run, NeoPilotRunExecution

class NeoPilotConfig(TypedDict):
    """
    NeoPilot config for CrewAIAgent
...
class CrewAIAgent(Agent):
    """
    CrewAIAgent lets you define your agent for use with NeoPilot.
    ...
    `description` can also be provided. This is used when NeoPilot is dynamically routing requests 
    to the agent.
    ...
    ```python
    from neopilot import CrewAIAgent
    ...
    def __init__(
        ...
        self.crew = crew
        self.flow = flow
        self.neopilot_config = neopilot_config or {}
    ...
    async def execute_flow( # pylint: disable=too-many-arguments,unused-argument,too-many-locals
        ...
        **kwargs,
        ...
        merge_state = self.neopilot_config.get("merge_state", crewai_flow_default_merge_state)

        crewai_flow_messages = neopilot_messages_to_crewai_flow(messages)
        ...
        execution: NeoPilotRunExecution = NeoPilotRunExecution(
            thread_id=thread_id,
            agent_name=self.name,
            run_id=run_id,
            should_exit=False,
        ...
        async for event in neopilot_run(
        ...
        if "messages" in state:
            state["messages"] = crewai_flow_messages_to_neopilot(state["messages"])
        ...
        if self.flow and self.flow._persistence: # pylint: disable=protected-access
            try:
                ...
                if "messages" in stored_state and stored_state["messages"]:
                    try:
                        messages = crewai_flow_messages_to_neopilot(stored_state["messages"])
...
class ChatWithCrewFlow(Flow):
    ...
    async def chat(self):
        ...
        tools = [action for action in self.state["neopilot"]["actions"]
