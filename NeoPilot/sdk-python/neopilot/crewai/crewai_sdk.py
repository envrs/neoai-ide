"""
CrewAI integration for NeoPilot
"""
...
from neopilot.types import Message
from neopilot.logging import get_logger
from neopilot.runloop import queue_put, get_context_execution
from neopilot.protocol import (
    RuntimeEventTypes,
    RunStarted,
    RunFinished,
    RunError,
...
class NeoPilotProperties(BaseModel):
    """NeoPilot properties"""
    actions: List[Any] = Field(default_factory=list)

class NeoPilotState(FlowState):
    """NeoPilot state"""
    messages: List[Any] = Field(default_factory=list)
    neopilot: NeoPilotProperties = Field(default_factory=NeoPilotProperties)
...
async def neopilot_emit_state(state: Any) -> Literal[True]:
    """
    Emits intermediate state to NeoPilot. 
    Useful if you have a longer running node and you want to update the user with the current state of the node.

    To install the NeoPilot SDK, run:
    ...
    ```python
    from neopilot.crewai import neopilot_emit_state

    for i in range(10):
        await some_long_running_operation(i)
        await neopilot_emit_state({"progress": i})
    ```
    ...
    state = {
        k: v for k, v in state_as_dict.items() if k not in ["messages", "neopilot"]
    }
...
async def neopilot_emit_message(message: str) -> str:
    """
...
async def neopilot_emit_tool_call(*, name: str, args: Dict[str, Any]) -> str:
    """
...
async def neopilot_stream(response):
    """
    Stream litellm responses token by token to NeoPilot.
    ...
    if isinstance(response, ModelResponse):
        return _neopilot_stream_response(response)
    if isinstance(response, CustomStreamWrapper):
        return await _neopilot_stream_custom_stream_wrapper(response)
    raise ValueError("Invalid response type")


async def _neopilot_stream_custom_stream_wrapper(response: CustomStreamWrapper):
    message_id: str = ""
    tool_call_id: str = ""
    content = ""
    created = 0
...
def _neopilot_stream_response(response: ModelResponse):
    return response


async def neopilot_exit() -> Literal[True]:
    """
    ...
    ```python
    from neopilot.crewai import neopilot_exit
...
async def neopilot_predict_state(
    ...
    Parameters
    ----------
    config : Dict[str, NeoPilotPredictStateConfig]
...
def neopilot_messages_to_crewai_flow(messages: List[Message]) -> List[Any]:
    """
    Convert NeoPilot messages to CrewAI Flow messages
    """
    result = []
...
def crewai_flow_messages_to_neopilot(messages: List[Dict]) -> List[Message]: # pylint: disable=too-many-branches
    """
    Convert CrewAI Flow messages to NeoPilot messages
    """
    result = []
    tool_call_names = {}
