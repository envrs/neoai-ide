...
class FlowInputState(BaseModel):
    ...
    tools: List[Dict[str, Any]] = [] # NeoPilot tool format: name, description, parameters
    conversation_history: List[Dict[str, str]] = [] # Full conversation history (persisted between runs)


class NeoPilotToolCallEvent(BaseEvent):
    """Event emitted when a tool call is made through NeoPilot"""
    type: str = "neopilot_frontend_tool_call"
...
class NeoPilotStateUpdateEvent(BaseEvent):
    """Event for state updates in NeoPilot"""
    type: str = "neopilot_state_update"
    tool_name: str
    args: dict[str, Any]
    timestamp: str = Field(default_factory=lambda: datetime.datetime.now().isoformat())
...
def create_tool_proxy(tool_name):
    def tool_proxy(**kwargs):
        event = NeoPilotToolCallEvent(tool_name=tool_name, args=kwargs)
...
class NeoPilotFlow(Flow[S], Generic[S]): # Make it generic
    ...
    def kickoff(self, state: Optional[S] = None, inputs: Optional[Dict[str, Any]] = None):
        ...
        if isinstance(inputs, dict) and "tools" in inputs:
            ...
            NeoPilotFlow._tools_from_input = inputs.get("tools", [])
            print(f"Tools from inputs dict: {NeoPilotFlow._tools_from_input}")
    ...
    def get_available_tools(self) -> List[Dict[str, Any]]:
        ...
        elif NeoPilotFlow._tools_from_input:
            raw_tools = NeoPilotFlow._tools_from_input
    ...
    def handle_tool_responses(
        ...
        if need_followup:
            ...
            # Add tool call results as messages (NeoPilot might do this differently, adjust if needed)
            # For OpenAI, tool results are typically added with role 'tool'
            # This part might need alignment with how NeoPilot expects tool results to be fed back.
            # The current [create_tool_proxy](cci:1://file:///Users/croonnicola/Downloads/agentic_chat/src/agentic_chat/neopilot_integration.py:22:0-42:21) returns a string. This string becomes the 'content'
...
def register_tool_call_listener():
    @crewai_event_bus.on(NeoPilotToolCallEvent)
    def on_tool_call_event(source, event):
        print(f"Received NeoPilot tool call event: Tool: {event.tool_name}, Args: {event.args}, Time: {event.timestamp}")
        pass
...
# Example: emit_neopilot_state_update_event("write_document", {"document": state.data["document"]})
def emit_neopilot_state_update_event(tool_name: str, args: dict[str, Any]):
    event = NeoPilotStateUpdateEvent(tool_name=tool_name, args=args)
    crewai_event_bus.emit(None, event=event)