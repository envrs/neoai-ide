"""
LangChain specific utilities for NeoPilot
"""
...
class NeoPilotProperties(TypedDict):
    """NeoPilot state"""
    actions: List[Any]
    context: List[CopilotContextItem]

class NeoPilotState(MessagesState):
    """NeoPilot state"""
    neopilot: NeoPilotProperties


def neopilot_messages_to_langchain(
        use_function_call: bool = False
    ) -> Callable[[List[Message]], List[BaseMessage]]:
    """
    Convert NeoPilot messages to LangChain messages
    """
    def _neopilot_messages_to_langchain(messages: List[Message]) -> List[BaseMessage]:
        result = []
    ...
    return _neopilot_messages_to_langchain


def langchain_messages_to_neopilot(
        messages: List[BaseMessage]
    ) -> List[Message]:
    """
    Convert LangChain messages to NeoPilot messages
    """
    result = []
    tool_call_names = {}
...
def neopilot_customize_config(
    ...
    config = neopilot_customize_config(
        config,
        emit_messages=False,
    ...
    if emit_all is not None:
        warnings.warn(
            ...
            "NeoPilot will now emit all messages and tool calls by default.",
            DeprecationWarning,
            stacklevel=2
        )
    ...
    if emit_all is True:
        ...
        if emit_tool_calls is not None:
            metadata["neopilot:emit-tool-calls"] = emit_tool_calls
        if emit_messages is not None:
            metadata["neopilot:emit-messages"] = emit_messages

    if emit_intermediate_state:
        metadata["neopilot:emit-intermediate-state"] = emit_intermediate_state
...
async def neopilot_exit(config: RunnableConfig):
    """
    ...
    ```python
    from neopilot.langgraph import neopilot_exit

    def my_node(state: Any):
        await neopilot_exit(config)
        return state
    ```
...
async def neopilot_emit_state(config: RunnableConfig, state: Any):
    """
    ...
    ```python
    from neopilot.langgraph import neopilot_emit_state

    for i in range(10):
        await some_long_running_operation(i)
        await neopilot_emit_state(config, {"progress": i})
    ```
    ...
    await adispatch_custom_event(
        "neopilot_manually_emit_intermediate_state",
        state,
        config=config,
    )
    await asyncio.sleep(0.02)
...
async def neopilot_emit_message(config: RunnableConfig, message: str):
    """
    ...
    ```python
    from neopilot.langgraph import neopilot_emit_message

    message = "Step 1 of 10 complete"
    await neopilot_emit_message(config, message)
...
async def neopilot_emit_tool_call(config: RunnableConfig, *, name: str, args: Dict[str, Any]):
    """
    Manually emits a tool call to NeoPilot.

    ```python
    from neopilot.langgraph import neopilot_emit_tool_call

    await neopilot_emit_tool_call(config, name="SearchTool", args={"steps": 10})
    ```
...
def neopilot_interrupt(
    ...
    response = interrupt({
        "__neopilot_interrupt_value__": interrupt_values,
        "__neopilot_messages__": [interrupt_message]
    })
