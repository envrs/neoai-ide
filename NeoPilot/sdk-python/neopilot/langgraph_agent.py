...
from .langgraph import neopilot_messages_to_langchain, langchain_messages_to_neopilot
...
class NeoPilotConfig(TypedDict):
    """
...
class LangGraphAgent(Agent):
    ...
    neopilot_config : Optional[NeoPilotConfig]
    ...
    def __init__(
        ...
        if merge_state is not None:
            logger.warning("Warning: merge_state is deprecated, use neopilot_config instead")
        ...
        self.thread_state = {}
        if neopilot_config is not None:
            self.merge_state = neopilot_config.get("merge_state")
        ...
        self.convert_messages = (
            neopilot_config.get("convert_messages")
            if neopilot_config
            else None
        ) or neopilot_messages_to_langchain(use_function_call=False)
    ...
    async def _stream_events( # pylint: disable=too-many-locals
        ...
        try:
            async for event in stream:
                ...
                emit_intermediate_state = metadata.get("neopilot:emit-intermediate-state")
                manually_emit_intermediate_state = (
                    event_type == "on_custom_event" and
                    event["name"] == "neopilot_manually_emit_intermediate_state"
                )
    ...
    def _emit_state_sync_event(
        ...
        *,
        ...
        if not include_messages:
            ...
            state = {
                **state,
                "messages": langchain_messages_to_neopilot(state.get("messages", []))
            }
    ...
    def get_schema_keys(self, config):
        CONSTANT_KEYS = ['neopilot', 'messages']
        ...
        try:
            ...
            # We add "neopilot" and "messages" as they are always sent and received.
    ...
    def get_interrupt_event(self, value):
        if not isinstance(value, str) and "__neopilot_interrupt_value__" in value:
            ev_value = value["__neopilot_interrupt_value__"]
            return langchain_dumps({
                "event": "on_neopilot_interrupt",
                "data": { "value": ev_value if isinstance(ev_value, str) else json.dumps(ev_value), "messages": langchain_messages_to_neopilot(value["__neopilot_messages__"]) }
            }) + "\n"
        else:
            return langchain_dumps({
                "event": "on_interrupt",
