...
class CustomEventNames(Enum):
    """Custom event names for NeoPilot"""
    ManuallyEmitMessage = "neopilot_manually_emit_message"
    ManuallyEmitToolCall = "neopilot_manually_emit_tool_call"
    ManuallyEmitState = "neopilot_manually_emit_intermediate_state"
...
class LangGraphAGUIAgent(LangGraphAgent):
    def __init__(self, *, name: str, graph: CompiledStateGraph, description: Optional[str] = None, config: Union[Optional[RunnableConfig], dict] = None):
        ...
        self.constant_schema_keys = self.constant_schema_keys + ["neopilot"]

    def _dispatch_event(self, event) -> str:
        """Override the dispatch event method to handle custom NeoPilot events and filtering"""
        ...
        if event.type == EventType.CUSTOM:
            ...
            if custom_event.name == "neopilot_exit":
                return super()._dispatch_event(
                    CustomEvent(
        ...
        if raw_event:
            ...
            if "neopilot:emit-tool-calls" in metadata:
                if metadata["neopilot:emit-tool-calls"] is False and is_tool_event:
                    return ""  # Don't dispatch this event
            
            if "neopilot:emit-messages" in metadata:
                if metadata["neopilot:emit-messages"] is False and is_message_event:
                    return ""  # Don't dispatch this event
    ...
    async def _handle_single_event(self, event: Any, state: State) -> AsyncGenerator[str, None]:
        ...
        if event.get("event") == LangGraphEventTypes.OnChatModelStream.value:
            predict_state_metadata = event.get("metadata", {}).get("neopilot:emit-intermediate-state", [])
    ...
    def langgraph_default_merge_state(self, state: State, messages: List[BaseMessage], input: Any) -> State:
        """Override to add NeoPilot actions to the state"""
        merged_state = super().langgraph_default_merge_state(state, messages, input)
        # Extract tools from the merged state and add them as NeoPilot actions
