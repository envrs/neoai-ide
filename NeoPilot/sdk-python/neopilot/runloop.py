...
class NeoPilotRunExecution(TypedDict):
    """
    NeoPilot Run Execution
    """
    thread_id: str
    agent_name: str
    run_id: str
...
def get_context_execution() -> NeoPilotRunExecution:
    """
    Get the execution from this task's context.
    """
    return cast(NeoPilotRunExecution, _CONTEXT_EXECUTION.get())

def set_context_execution(execution: NeoPilotRunExecution) -> contextvars.Token:
    """
...
async def neopilot_run(
        fn: Callable,
        *,
        execution: NeoPilotRunExecution
):
    """
