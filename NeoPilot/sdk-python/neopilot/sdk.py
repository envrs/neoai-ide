...
NEOPILOT_SDK_VERSION = __version__
...
class NeoPilotContext(TypedDict):
    """
    NeoPilot Context
...
NeoPilotSDKContext = NeoPilotContext


class NeoPilotRemoteEndpoint:
    """
    ...
    Parameters
    ----------
    actions : Optional[Union[List[Action], Callable[[NeoPilotContext], List[Action]]]]
        The actions to make available to the Copilot.
    agents : Optional[Union[List[Agent], Callable[[NeoPilotContext], List[Agent]]]]
    ...
    def __init__(
        ...
        *,
        actions: Optional[
            Union[
                List[Action],
                Callable[[NeoPilotContext], List[Action]]
            ]
        ] = None,
        agents: Optional[
            Union[
                List[Agent],
                Callable[[NeoPilotContext], List[Agent]]
            ]
        ] = None,
    ):
        ...
        if isinstance(agents, list):
            ...
            for agent in agents:
                if isinstance(agent, LangGraphAgent):
                    raise ValueError(
                        "LangGraphAgent should be instantiated using LangGraphAGUIAgent. Refer to https://docs.neopilot.ai/langgraph for more information.")
        

    def info(
        self,
        *,
        context: NeoPilotContext
    ) -> InfoDict:
        """
        ...
        return {
            "actions": actions_list,
            "agents": agents_list,
            "sdkVersion": NEOPILOT_SDK_VERSION
        }
...
# Alias for backwards compatibility
class NeoPilotSDK(NeoPilotRemoteEndpoint):
    """Deprecated: Use NeoPilotRemoteEndpoint instead. This class will be removed in a future version."""

    def __init__(self, *args, **kwargs):
        warnings.warn(
            "NeoPilotSDK is deprecated since version 0.1.31. "
            "Use NeoPilotRemoteEndpoint instead.",
            DeprecationWarning,
            stacklevel=2
        )
